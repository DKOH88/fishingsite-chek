...
# =============================================================================
# 🚀 헤르메스호 API(Packet) 봇 - (좌석 선택 활성화 + GUI 연동)
# =============================================================================
#
# [설명]
# Requests(패킷) 방식으로 '헤르메스호'를 예약합니다.
# **좌석 선택**: AJAX 좌석 조회 후 선호 좌석을 선택합니다.
# **GUI 연동**: launcher로부터 설정 파일 경로를 인자로 받아 동적으로 실행됩니다.
#
# [사용법]
# python "헤르메스호_API.py" --config [config_file_path]
# =============================================================================

# 1. 헤르메스호 기본 정보 (hermes.thefishing.kr / v5.2_seat1)
BASE_URL = "http://hermes.thefishing.kr/_core/module/reservation_boat_v5.2_seat1"
PA_N_UID = "5579"  # 헤르메스호 고유 ID
    ...
    "20261001": [
        {
            ...
            "person_info": {
                ...
                "BI_TEL2": "1234",  # 뒷번호
                "BI_TEL3": "5678",  # 끝번호
                "seat_preference": ['15', '8', '7', '14', '1', '9', '6', '13', '2', '10', '5', '12', '3', '11', '4'], 
            },
        },
    ]
}
...
def build_session():
    ...
    s.headers.update({
        ...
        "Origin": "http://hermes.thefishing.kr"
    })
...
def do_reservation(session, date, job):
    ...
    payload = [
        ("action", "insert"),
        ("link", f"/_core/module/reservation_boat_v5.2_seat1/popu2.step2.php"),  # v5.2_seat1 모듈
        ("temp_bi_stat", "확인"),
        ("date", date),
        ("PA_N_UID", PA_N_UID),
        ("PH_N_UID", ph_n_uid),
    ...
    # 📌 [중요] v5.2_seat1 모듈은 좌석을 POST 데이터에 포함하지 않음!
    # 좌석은 클릭 시 서버 세션에 저장되며, 최종 예약 시 세션에서 읽어옴.
    # ** API 방식으로는 세션 기반 좌석 선택이 불가능하므로, 좌석 선택 없이 진행 **
    # (서버가 자동으로 빈 좌석을 할당할 가능성 있음)
    if selected_seats:
        print(f"   ⚠️ 좌석 {selected_seats} 감지되었으나, API 방식은 세션 기반 좌석 선택 불가")
        print(f"   ℹ️ 좌석 정보를 POST 데이터에 포함하지 않습니다 (서버 세션 방식)")
