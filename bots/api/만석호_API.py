...
# =============================================================================
# 🚀 만석호 API(Packet) 봇 - (좌석 선택 활성화 + GUI 연동)
# =============================================================================
#
# [설명]
# Requests(패킷) 방식으로 '만석호'를 예약합니다.
# **좌석 선택**: HTML 좌석 조회 후 선호 좌석을 선택합니다.
# **GUI 연동**: launcher로부터 설정 파일 경로를 인자로 받아 동적으로 실행됩니다.
#
# [사용법]
# python "만석호_API.py" --config [config_file_path]
# =============================================================================

# 1. 만석호 기본 정보 (mscufishing.com / v5.2_seat1 - popup.step1.php 사용)
    ...
    "20261001": [
        {
            ...
            "person_info": {
                ...
                "BI_TEL2": "1234",  # 뒷번호
                "BI_TEL3": "5678",  # 끝번호
                "seat_preference": ['1', '15', '2', '9', '8', '10', '7', '14', '3', '11', '6', '13', '4', '12', '5'], 
            },
        },
    ]
}
...
def build_session():
    ...
    s.headers.update({
        ...
        "Origin": "http://www.mscufishing.com"
    })
...
def do_reservation(session, date, job):
    ...
    payload = [
        ("action", "insert"),
        ("link", f"/_core/module/reservation_boat_v5.2_seat1/popup.step2.php"),  # v5.2_seat1 모듈
        ("temp_bi_stat", "확인"),
        ("date", date),
        ("PA_N_UID", PA_N_UID),
        ("PH_N_UID", ph_n_uid),
    ...
    # ═══════════════════════════════════════════════════════════════
    # 만석호 3단계 예약 프로세스 (STEP 1 → STEP 2 → STEP 3)
    # ═══════════════════════════════════════════════════════════════
    ...
    # ─────────────────────────────────────────────────────────────
    step_start = time.time()
    url_step2 = f"{BASE_URL}/popup.step2.php?date={date}&PA_N_UID={PA_N_UID}&PS_N_UID={ps_uid}"
    print(f"\n🚀 [STEP 2] 두 번째 페이지 로드...")
    ...
    # ─────────────────────────────────────────────────────────────
    step_start = time.time()
    url_step3 = f"{BASE_URL}/popup.step3.php?date={date}&PA_N_UID={PA_N_UID}&PS_N_UID={ps_uid}"
    print(f"\n🎯 [STEP 3] 최종 성공 확인 중...")
