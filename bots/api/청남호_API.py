...
# =============================================================================
# 🚀 청남호 API(Packet) 봇 - (좌석 선택 활성화 + GUI 연동)
# =============================================================================
#
# [설명]
# Requests(패킷) 방식으로 '청남호'를 예약합니다.
# **좌석 선택**: AJAX 좌석 조회 후 선호 좌석을 선택합니다.
# **GUI 연동**: launcher로부터 설정 파일 경로를 인자로 받아 동적으로 실행됩니다.
#
# [사용법]
# python "청남호_API.py" --config [config_file_path]
# =============================================================================

# 1. 청남호 기본 정보 (chungnamho.com / v5.2_seat1)
BASE_URL = "http://www.chungnamho.com/_core/module/reservation_boat_v5.2_seat1"
PA_N_UID = "1441"  # 청남호 고유 ID
    ...
    "20261001": [
        {
            ...
            "person_info": {
                ...
                "BI_TEL2": "1234",  # 뒷번호
                "BI_TEL3": "5678",  # 끝번호
                "seat_preference": [],  # 빈 배열 = 좌석 선택 비활성화 (서버 자동 배정)
            },
        },
    ]
}
...
def build_session():
    ...
    s.headers.update({
        ...
        "Origin": "http://www.chungnamho.com"
    })
...
def do_reservation(session, date, job):
    ...
    try:
        naun_int = int(naun)
        if naun_int < seats_needed:
            print(f"   ⚠️ 잔여석({naun}) < 요청({seats_needed}) → {naun}석으로 자동 조정!")
            seats_needed = naun_int
        elif naun_int == 0:
            print(f"   ❌ 잔여석 0석 - 예약 불가!")
            return False
    except ValueError:
        print(f"   ⚠️ naun 파싱 오류: {naun}")
    
    print(f"⏱️ [Step 1-2] 어종 선택 및 정보 로드: {time.time() - step_start:.4f}초")

    # ─────────────────────────────────────────────────────────────
    # Step 1-3: AJAX 좌석 조회 및 선택 (좌석 선택 활성화!)
    # ─────────────────────────────────────────────────────────────
    step_start = time.time()
    ...
    if seat_preference:
        print(f"\n🪑 [Step 1-3] AJAX 좌석 조회 중...")
        print(f"   📋 선호 좌석: {seat_preference}")
        
        # AJAX로 좌석 정보 조회
        ajax_url = f"{BASE_URL}/ajax/bi_seat.ajax.php"
        ajax_data = {
            "date": date,
            "PA_N_UID": PA_N_UID,
            "PS_N_UID": ps_uid,
        ...
        try:
            ...
            ajax_res = session.post(ajax_url, data=ajax_data, timeout=REQUEST_TIMEOUT)
            print(f"   ✅ AJAX 응답: Status={ajax_res.status_code}, Size={len(ajax_res.text)}bytes ({time.time()-ajax_start:.4f}초)")
            
            # 좌석 정보 파싱 (JSON 또는 HTML)
            available_seats = []
            try:
                # JSON 응답인 경우
                seat_data = ajax_res.json()
                if isinstance(seat_data, list):
                    for seat in seat_data:
                        if seat.get("status") == "available" or seat.get("BI_N_UID") == "0":
                            available_seats.append(seat.get("seat_num") or seat.get("number"))
                elif isinstance(seat_data, dict):
                    for key, val in seat_data.items():
                        if val.get("status") == "available" or val.get("BI_N_UID") == "0":
                            available_seats.append(val.get("seat_num") or key)
            except:
                # HTML 응답인 경우 - 빈 좌석 파싱
                from bs4 import BeautifulSoup
                soup = BeautifulSoup(ajax_res.text, "html.parser")
                
                # 방법 1: class="seat" 또는 data-seat 속성
                seat_elements = soup.find_all(class_="seat")
                if not seat_elements:
                    seat_elements = soup.find_all(attrs={"data-seat": True})
                
                for elem in seat_elements:
                    # 예약된 좌석은 보통 class에 "reserved", "booked", "selected" 등이 있음
                    classes = elem.get("class", [])
                    if not any(c in classes for c in ["reserved", "booked", "selected", "disabled"]):
                        seat_num = elem.get("data-seat") or elem.get("data-num") or elem.text.strip()
                        if seat_num and seat_num.isdigit():
                            available_seats.append(int(seat_num))
                
                # 방법 2: input checkbox/radio로 좌석 선택하는 경우
                if not available_seats:
                    seat_inputs = soup.find_all("input", {"name": re.compile(r"seat|BI_SEAT", re.I)})
                    for inp in seat_inputs:
                        if not inp.get("disabled") and not inp.get("checked"):
                            seat_num = inp.get("value")
                            if seat_num and seat_num.isdigit():
                                available_seats.append(int(seat_num))
            
            print(f"   📊 조회된 빈 좌석: {available_seats if available_seats else '파싱 실패 또는 없음'}")
            
            # 선호 좌석 중 빈 좌석 선택
            if available_seats:
                for pref in seat_preference:
                    if pref in available_seats and len(selected_seats) < seats_needed:
                        selected_seats.append(pref)
                
                # 선호 좌석이 다 차있으면 아무 빈 좌석 선택
                if len(selected_seats) < seats_needed:
                    for seat in available_seats:
                        if seat not in selected_seats and len(selected_seats) < seats_needed:
                            selected_seats.append(seat)
            
            print(f"   ✅ 선택된 좌석: {selected_seats if selected_seats else '없음 (서버 자동 배정)'}")
            
        except Exception as e:
            print(f"   ⚠️ AJAX 좌석 조회 실패: {e}")
            print(f"   ℹ️ 좌석 선택 생략 - 서버 자동 배정")
        
        print(f"⏱️ [Step 1-3] 좌석 조회 완료: {time.time() - step_start:.4f}초")
    else:
    ...
    payload = [
        ("action", "insert"),
        ("link", f"/_core/module/reservation_boat_v5.2_seat1/popup.step2.php"), 
        ("temp_bi_stat", "확인"),
        ("date", date),
        ("PA_N_UID", PA_N_UID),
        ("PH_N_UID", ph_n_uid),
    ...
    if selected_seats:
        ...
        print(f"   🪑 좌석 정보 추가: {selected_seats}")
    ...
    # ─────────────────────────────────────────────────────────────
    # Step 4: Step 1 → Step 2 페이지 진입 (Action)
    # ─────────────────────────────────────────────────────────────
    step_start = time.time()
    url_action = f"{BASE_URL}/action/popup.step1.action.php"
    print(f"\n📡 [Step 4] POST Step 1 제출 → Step 2 진입...")
    print(f"   🔗 URL: {url_action}")
    
    session.headers.update({"Referer": url_2})
    
    # Step 2 페이지 URL 저장용 (Referer에 사용)
    redirect_url = None
    ...
    try:
        ...
        print(f"⏱️ [Step 4] Step 2 진입 완료: {http_duration:.4f}초")
        ...
        if "정상적으로 예약해 주십시오" in response_text:
            print("⚠️ [Step 4] 예약 오류 - 처음부터 다시 시도 필요")
            return "RETRY_FULL"
        elif "이미" in response_text or "불가능" in response_text:
            print("⚠️ [Step 4] 좌석 선점 실패 - 즉시 재시도")
            return "RETRY_IMMEDIATE"
        # Step 2 페이지 도달 확인 (JavaScript redirect 감지)
        if "location.replace" in response_text or "location.href" in response_text:
            print(f"   ⚙️ JavaScript 리다이렉트 감지 → Step 2 페이지로 이동...")
            
            # 최종 시도: Step 2 GET이 세션 플래그를 설정하는 필수 단계일 수 있음
            # 리다이렉트 URL 추출
            import re
            redirect_match = re.search(r"location\.(replace|href)\(['\"]([^'\"]+)['\"]", response_text)
            if redirect_match:
                redirect_path = redirect_match.group(2)
                # 상대 경로를 절대 URL로 변환
                if redirect_path.startswith('/'):
                    redirect_url = f"http://www.chungnamho.com{redirect_path}"
                else:
                    redirect_url = f"{BASE_URL}/{redirect_path}"
            else:
                # 기본 Step 2 URL
                redirect_url = f"{BASE_URL}/popup.step2.php"
            
            # 필수 쿼리 파라미터 추가 (서버가 세션 데이터를 로드하기 위해 필요)
            if '?' not in redirect_url:
                redirect_url = f"{redirect_url}?date={date}&PA_N_UID={PA_N_UID}&PS_N_UID={ps_uid}"
            
            print(f"   🔗 리다이렉트 URL: {redirect_url}")
            
            # Step 2 페이지 GET 요청 (세션 유지하면서)
            session.headers.update({"Referer": url_action})
            r_step2 = session.get(redirect_url, timeout=REQUEST_TIMEOUT)
            
            print(f"   ✅ Step 2 페이지 로드: Status={r_step2.status_code}, Size={len(r_step2.text)}bytes")
            
            if r_step2.status_code != 200:
                print(f"   ❌ Step 2 페이지 로드 실패")
                return "RETRY_FULL"
            
            response_text = r_step2.text
            print(f"   ✅ Step 2 페이지 도달 성공!")
            
            # Step 2 페이지에서 BI_N_UID 또는 세션 기반 확인
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(response_text, "html.parser")
            
            bi_n_uid = None
            bi_input = soup.find("input", {"name": "BI_N_UID"})
            if bi_input:
                bi_n_uid = bi_input.get("value")
                print(f"   📝 BI_N_UID 파싱: {bi_n_uid}")
            else:
                # 세션 기반 확인
                action_input = soup.find("input", {"name": "action"})
                if action_input and action_input.get("value") == "update":
                    bi_n_uid = "SESSION_BASED"
                    print(f"   ℹ️ 세션 기반 시스템 감지 (Step 2 페이지 방문 완료)")
            
            if not bi_n_uid:
                print("   ⚠️ BI_N_UID를 찾을 수 없고 세션 기반도 아닙니다.")
                return "RETRY_FULL"
            
        elif "step2" in response_text.lower() or "예약2단계" in response_text:
            print(f"   ✅ Step 2 페이지 도달 성공! (직접 응답)")
            
            # Step 2 페이지에서 BI_N_UID 파싱 시도
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(response_text, "html.parser")
            ...
            if bi_input:
                ...
                if action_input and action_input.get("value") == "update":
                    bi_n_uid = "SESSION_BASED"
                    print(f"   ℹ️ 세션 기반 시스템 감지")
            
            if not bi_n_uid:
                print("   ⚠️ BI_N_UID를 찾을 수 없고 세션 기반도 아닙니다.")
                return "RETRY_FULL"
        else:
            print(f"⚠️ [Step 4] Step 2 페이지 도달 실패")
            print(f"   📄 응답: {response_text[:300]}...")
            return "RETRY_FULL"
        
        # ─────────────────────────────────────────────────────────────
        # Step 5: Step 2 → Step 3 최종 예약 신청 (청남호 전용)
        # ─────────────────────────────────────────────────────────────
        step_start = time.time()
        ...
        # Step 2 페이로드: 세션 기반 vs BI_N_UID 기반
        if bi_n_uid == "SESSION_BASED":
            # 세션 기반: Step 2 페이지의 모든 input 필드 포함
            step2_payload = {}
            
            # Step 2 페이지 HTML에서 모든 input 파싱
            from bs4 import BeautifulSoup
            soup_step2 = BeautifulSoup(response_text, "html.parser")
            
            # 폼 내의 모든 input (hidden, text, textarea 등) 수집
            form = soup_step2.find("form")
            if form:
                # Hidden inputs
                for inp in form.find_all("input", {"type": "hidden"}):
                    ...
                    value = inp.get("value", "")
                    if name:
                        step2_payload[name] = value
                        print(f"      - {name}: {value}")
                
                # Text inputs (BI_MEMO 등)
                ...
                # Textarea (BI_MEMO가 textarea일 수도)
                for textarea in form.find_all("textarea"):
                    name = textarea.get("name")
                    value = textarea.text.strip() if textarea.text else ""
                    if name:
                        step2_payload[name] = value
                        print(f"      - {name}: {value}")
            
            # 기본값 설정 (파싱 실패 대비)
            if not step2_payload:
                step2_payload = {
                    "action": "update",
                    "link": "/_core/module/reservation_boat_v5.2_seat1/popup.step3.php",
                    "BI_MEMO": ""
                }
            
            # BI_MEMO 필드 강제 추가 (브라우저 Golden Payload에 있음)
            if "BI_MEMO" not in step2_payload:
                step2_payload["BI_MEMO"] = ""
                print(f"      - BI_MEMO: (강제 추가)")
            
            print(f"   📋 세션 기반 페이로드: {len(step2_payload)}개 필드")
        else:
            # BI_N_UID 기반: Step 1의 모든 정보 + BI_N_UID
            step2_payload = {
                "action": "insert",
                "date": date,
                "PA_N_UID": PA_N_UID,
            ...
            if selected_seats:
                for seat in selected_seats:
                    step2_payload[f"BI_SEAT[]"] = str(seat)
            
            print(f"   📋 Step 2 페이로드: {len(step2_payload)}개 필드")
            print(f"   📋 주요: BI_N_UID={bi_n_uid}, BI_IN={seats_needed}, name={info['BI_NAME']}")
        
        # Step 2 → Step 3 제출 (Referer를 Step 2 페이지로 설정)
        # Step 2 페이지를 방문했으므로 그 URL을 Referer로 사용
        if redirect_url:
            step2_referer = redirect_url
        else:
            step2_referer = f"{BASE_URL}/popup.step2.php?date={date}&PA_N_UID={PA_N_UID}"
        
        session.headers.update({"Referer": step2_referer})
        print(f"   🔗 Referer: {step2_referer}")
        
        res2 = session.post(url_step2_action, data=step2_payload, timeout=REQUEST_TIMEOUT)
        http_duration = time.time() - step_start
        
        print(f"   ✅ 응답 수신: Status={res2.status_code}, Size={len(res2.text)}bytes")
        print(f"⏱️ [Step 5] 최종 전송 완료: {http_duration:.4f}초")
        
        if res2.status_code == 200:
            response_text2 = res2.text.strip()
            print(f"\n📋 [응답 분석]")
            print(f"   📄 응답 내용: {response_text2[:200]}..." if len(response_text2) > 200 else f"   📄 응답 내용: {response_text2}")
            
            # 최종 성공/실패 메시지 확인
            if "예약 신청이 완료되었습니다" in response_text2 or "step3" in response_text2.lower() or "예약완료" in response_text2:
                print("🎉 [결과] 예약 성공!")
            elif "정상적으로 예약해 주십시오" in response_text2:
                print("⚠️ [결과] 예약 오류 - 처음부터 다시 시도 필요")
                return "RETRY_FULL"
            elif "이미" in response_text2 or "불가능" in response_text2:
                print("⚠️ [결과] 좌석 선점 실패 - 즉시 재시도")
                return "RETRY_IMMEDIATE"
            else:
                print("ℹ️ [결과] 응답 확인 필요 (Step 3 도달 가능성)")
            ...
            print(f"❌ [Step 5 전송실패] Status: {res2.status_code}")
            if res2.status_code in [502, 503]:
                return "RETRY_FULL"
            return False
