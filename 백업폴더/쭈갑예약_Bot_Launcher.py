...
class FishingLauncher:
    ...
    def create_widgets(self):
        ...
        # Row 0: Name, Depositor, Reservation Info Button
        ttk.Label(self.lf_config, text="예약자 이름").grid(row=0, column=0, sticky="e", padx=5, pady=5)
        self.entry_name = ttk.Entry(self.lf_config, width=25)
        self.entry_name.insert(0, "홍길동")
        ...
        self.entry_depositor = ttk.Entry(self.lf_config, width=25)
        self.entry_depositor.insert(0, "홍길동")
        self.entry_depositor.grid(row=0, column=3, sticky="w", padx=5)
        
        ttk.Button(self.lf_config, text="📅 예약일 정보", command=self.open_reservation_info).grid(row=0, column=4, padx=10)
        
        # Row 1: Phone + Execution Time
        ttk.Label(self.lf_config, text="휴대폰 번호").grid(row=1, column=0, sticky="e", padx=5, pady=5)
        self.entry_phone = ttk.Entry(self.lf_config, width=25)
        self.entry_phone.insert(0, "010-0000-0000")
        self.entry_phone.grid(row=1, column=1, sticky="w", padx=5)
        
        # Execution Time (in Row 1)
        ttk.Label(self.lf_config, text="실행 시간").grid(row=1, column=2, sticky="e", padx=5)
        frame_time = ttk.Frame(self.lf_config)
        frame_time.grid(row=1, column=3, sticky="w", padx=5)
        
        self.cb_hour = ttk.Combobox(frame_time, values=[f"{i:02d}" for i in range(24)], width=3, state="readonly", height=24)
        self.cb_hour.set("09")
        self.cb_hour.pack(side="left")
        ttk.Label(frame_time, text="시").pack(side="left")
        
        self.cb_min = ttk.Entry(frame_time, width=3)
        self.cb_min.insert(0, "00")
        self.cb_min.pack(side="left")
        ttk.Label(frame_time, text="분").pack(side="left")
        
        self.cb_sec = ttk.Entry(frame_time, width=3)
        self.cb_sec.insert(0, "00")
        ...
        self.slots_container.grid(row=2, column=0, columnspan=4, sticky="w", padx=5, pady=2)
        
        # Add/Remove buttons frame
        frame_slot_btns = ttk.Frame(self.lf_config)
        frame_slot_btns.grid(row=2, column=4, sticky="ne", padx=5)
        ttk.Button(frame_slot_btns, text="➕ 추가", width=8, command=self.add_slot).pack(pady=2)
        ttk.Button(frame_slot_btns, text="➖ 제거", width=8, command=self.remove_slot).pack(pady=2)
        ttk.Button(frame_slot_btns, text="📋 일괄복사", width=8, command=self.copy_first_slot).pack(pady=2)
        ...
        ttk.Checkbutton(self.lf_config, text="🚀 Test Mode (시간 무시/즉시 실행)", variable=self.var_test_mode).grid(row=3, column=0, columnspan=3, sticky="w", padx=5, pady=5)
    ...
    def add_slot(self):
        """Add a new slot row"""
        i = len(self.slots)
        default_y, default_m, default_d = "2026", "01", "01"
        ...
        cb_port = ttk.Combobox(frame_slot, values=self.port_list, width=13, state="readonly", height=15)
        ...
        # Date
        cb_year = ttk.Combobox(frame_slot, values=["2025", "2026"], width=5, state="readonly", height=10)
        cb_year.set(default_y)
        ...
        cb_day = ttk.Combobox(frame_slot, values=[f"{d:02d}" for d in range(1, 32)], width=3, state="readonly", height=31)
        cb_day.set(default_d)
        cb_day.pack(side="left")
        ttk.Label(frame_slot, text="일").pack(side="left", padx=(0, 10))

        # Person
        ttk.Label(frame_slot, text="인원:").pack(side="left")
        cb_person = ttk.Combobox(frame_slot, values=[str(p) for p in range(1, 6)], width=3, state="readonly")
        cb_person.set("1")
        cb_person.pack(side="left")
        ttk.Label(frame_slot, text="명").pack(side="left")
    ...
    def copy_first_slot(self):
        """Copy first checked slot settings to all other checked slots"""
        ...
        for i, slot in enumerate(self.slots):
            if slot["enable"].get() and i != first_idx:
                ...
                # Set date
                slot["year"].set(source_year)
                slot["month"].set(source_month)
                slot["day"].set(source_day)
        ...
        if copied_count > 0:
            self.log(f"📋 {first_idx+1}번 슬롯 설정을 {copied_count}개 슬롯에 복사 완료 ({source_port}/{source_provider}/{source_year}-{source_month}-{source_day}/{source_person}명)")
        else:
                    ...
                    if multi_data:
                        ...
                        for i, slot_data in enumerate(multi_data):
                            if i < len(self.slots):
                                ...
                                if saved_provider:
                                    # Try to find and set the provider (both clean and raw)
    ...
    def save_config(self, silent=False):
        ...
        data = {
            ...
            "target_date": t_date_legacy,
            "multi_instance": multi_instance_data,
            "target_time": t_time,
            "person_count": self.slots[0]['person'].get() if self.slots else "1",
    ...
    def start_bot(self):
        ...
        # Iterate through each slot directly from UI
        for i, slot in enumerate(self.slots):
            if not slot['enable'].get():
                continue
            ...
            self.log(f"🚀 Slot {i+1} 시작: {port} - {provider_clean} | {d_str} | {slot['person'].get()}명")
