...
class FishingLauncher:
    ...
    def create_widgets(self):
        ...
        frame_slot_btns.grid(row=2, column=4, sticky="n", padx=5, pady=(30, 0))
        ttk.Button(frame_slot_btns, text="➕ 추가", width=11, command=self.add_slot).pack(pady=2)
        ttk.Button(frame_slot_btns, text="➖ 제거", width=11, command=self.remove_slot).pack(pady=2)
        ttk.Button(frame_slot_btns, text="📋 일괄복사", width=11, command=self.copy_first_slot).pack(pady=2)
    ...
    def start_bot(self):
        ...
        for grid_idx, (i, slot) in enumerate(enabled_slots):
            ...
            self.log(f"🚀 Slot {i+1} 시작: {port} - {provider_clean} | {d_str} | {slot['person'].get()}명 | 위치:({win_x},{win_y})")
